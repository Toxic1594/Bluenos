    ...
    public class NpcPacketHandler : IPacketHandler
    {
        ...
        public void BuyShop(BuyPacket buyPacket)
        {
            ...
            switch (buyPacket.Type)
            {
                case BuyShopType.CharacterShop:
                    ...
                    if (sess == null || item == null || amount <= 0 || amount > 32000)
                    {
                        return;
                    }
                    ...
                    if (npc != null)
                    {
                        ...
                        if (npc.Shop.ShopSkills.Count > 0)
                        {
                            ...
                            if (Session.Character.Skills.Any(s =>
                                s.LastUse.AddMilliseconds(s.Skill.GetCooldown(Session.Character) * 100) > DateTime.Now))
                            {
                                Session.SendPacket(
                            ...
                            else
                            {
                                if (skillinfo.SkillVNum < 200)
                                {
                                    ...
                                    if (skillinfo.MinimumSwordmanLevel == 0 && skillinfo.MinimumArcherLevel == 0
                                                                            ...
                                                                            && skillinfo.MinimumWreastlerLevel == 0
                                                                            )
                                    ...
                                    else
                                    {
                                        switch (Session.Character.Class)
                                        {
                                            ...
                                            case ClassType.Wreastler:
                                                skillMiniumLevel = skillinfo.MinimumWreastlerLevel;
                                                break;
                        ...
                        else if (npc.Shop.ShopItems.Count > 0)
                        {
                            ...
                            if (shopItem == null || amount <= 0 || amount >= 1000)
                            {
                                return;
                            }
                            ...
                            for (short j = 3, i = 0; j < 82; j += 4, i++)
                            {
                                ...
                                if (qty[i] > 0)
                                {
                                    ...
                                    if (inv != null)
                                    {
                                        ...
                                        if (!inv.Item.IsTradable || ((inv.IsBound && (inv.Item.EquipmentSlot != EquipmentType.Armor && inv.Item.EquipmentSlot != EquipmentType.MainWeapon && inv.Item.EquipmentSlot != EquipmentType.SecondaryWeapon))))
                                        {
                                            Session.SendPacket(UserInterfaceHelper.GenerateMsg(
        ...
        public void PetMove(PtCtlPacket ptCtlPacket)
        {
            if (ptCtlPacket.PacketEnd == null)
            {
                ...
                foreach (ClientSession team in ServerManager.Instance.Sessions.Where(s =>
                    ...
                    {
                        team.SendPacket(team.Character.GenerateSay($"User {Session.Character.Name} tried a crash: PetMove",
                        12));
                    }
                Logger.Log.Warn($"user {Session.Character.Name} tried a Crash: PetMove");
                return;
            }
        ...
        public void SellShop(SellPacket sellPacket)
        {
            ...
            if (sellPacket.Amount.HasValue && sellPacket.Slot.HasValue)
            {
                ...
                int amount = sellPacket.Amount.Value;
                short slot = sellPacket.Slot.Value;
        ...
        private bool BuyValidate(ClientSession clientSession, KeyValuePair<long, MapShop> shop, short slot, int amount)
        {
