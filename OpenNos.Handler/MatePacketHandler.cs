    ...
    public class MatePacketHandler : IPacketHandler
    {
        ...
        public void Attack(SuctlPacket suctlPacket)
        {
            try
            {
                ...
                if (suctlPacket.MateTransportId == 0)
                {
        ...
        public void AttackCharacter(Mate attacker, NpcMonsterSkill skill, Character target)
        {
            ...
            if (target.Hp > 0 && attacker.Hp > 0)
            {
                ...
                int[] invisibleillusionista = target.GetBuff(BCardType.CardType.EffectSummon,
                ...
                if (sacrificio[0] != 0)
                {
                    int senderid = target.GetBuff(BCardType.CardType.DamageConvertingSkill, (byte)AdditionalTypes.DamageConvertingSkill.TransferInflictedDamage)[2];
                    if (senderid > 0)
                    {
                        ...
                        if (s != null)
                        {
                            ...
                            {
                                if (s.Character.IsInRange(target.PositionX, target.PositionY, 10))
                                {
                                    int dannoricevuto = 0;
                                    if (damage / 2 > s.Character.SwitchLevel() * 8)
                                    {
                                        ...
                                        target.Session.CurrentMapInstance?.Broadcast($"dm 1 {s.Character.CharacterId} {s.Character.SwitchLevel() * 8}");
                                    ...
                                    else
                                    {
                                        ...
                                        s.SendPacket(s.Character.GenerateStat());
                                        target.Session.CurrentMapInstance?.Broadcast($"dm 1 {s.Character.CharacterId} {(short)((damage / 2) * 0.75) * 2}");
                ...
                if (!isAlive && target.Session.HasCurrentMapInstance)
                {
                        ...
                        == true)
                    {
                        ...
                        if (target.SwitchLevel() + 10 >= Session.Character.SwitchLevel()
                            && Session.Character.SwitchLevel() <= target.SwitchLevel() - 10)
                        {
                        ...
                        {
                            target.Reputation -= target.SwitchLevel() * 50;
                            Session.Character.Reputation += target.SwitchLevel() * 50;
                ...
                Session.CurrentMapInstance?.Broadcast(StaticPacketHelper.SkillUsed(UserType.Npc,
                    attacker.MateTransportId, 1, target.CharacterId, 0, 12, 11, 200, attacker.MapX, attacker.MapY, isAlive,
