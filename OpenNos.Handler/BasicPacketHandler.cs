    ...
    public class BasicPacketHandler : IPacketHandler
    {
        ...
        public void CharacterOptionChange(CharacterOptionPacket characterOptionPacket)
        {
            switch (characterOptionPacket.Option)
            {
                ...
                case CharacterOption.PetRespawn:
                    Session.Character.PetRespawn = characterOptionPacket.IsActive;
                    Session.SendPacket(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey(Session.Character.PetRespawn ? "PET_SPAWN_ACTIVATE" : "PET_SPAWN_DEACTIVATE"), 0));
                    break;

                case CharacterOption.PartnerRespawn:
                    Session.Character.PartnerRespawn = characterOptionPacket.IsActive;
                    Session.SendPacket(UserInterfaceHelper.GenerateMsg(
                        Language.Instance.GetMessageFromKey(Session.Character.PartnerRespawn ? "PARTNER_SPAWN_ACTIVATE" : "PARTNER_SPAWN_DEACTIVATE"), 0));
                    break;
                                    ...
                                    switch (fInsPacket.Type)
                                    {
                                        ...
                                        case 34:
                                            ...
                                            CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage
                                            {
                                                ...
                                                Message =
                                                    $"{Session.Character.Name} and {otherSession.Character.Name} get Married! Congratulations from Bluenos Team! ",
        ...
        public void GetGift(GetGiftPacket getGiftPacket)
        {
            ...
            if (Session.Character.MailList.ContainsKey(giftId))
            {
                ...
                if (getGiftPacket.Type == 4 && mail.AttachmentVNum != null)
                {
                    ...
                    {
                        ItemInstance temp = new ItemInstance((short)mail.AttachmentVNum, mail.AttachmentAmount);
                        if (temp.Item.ItemType == ItemType.Box)
                        {
                            temp.Design = mail.AttachmentUpgrade;
                            temp.RarifyItem(Session, RarifyMode.Drop, RarifyProtection.None);
                            temp.Upgrade = 0;
                        }
                        else
                        {
                            temp.Rare = (sbyte)mail.AttachmentRarity;
                            temp.Upgrade = mail.AttachmentUpgrade;
                        }
                        ItemInstance newInv = Session.Character.Inventory.AddToInventory(temp)
                            .FirstOrDefault();
                        if (newInv != null)
                        {
                            if (newInv.Rare != 0)
            ...
            if (guriPacket != null)
            {
                ...
                else if (guriPacket.Type == 204)
                {
                    if (guriPacket.Argument == 0 && short.TryParse(guriPacket.User.ToString(), out short slot))
                    {
                        ...
                        {
                            if (shell.Item.ItemType != ItemType.Shell)
                            {
                                int duration = 365;
                ...
                else if (guriPacket.Type == 300)
                {
                    if (guriPacket.Argument == 8023 && short.TryParse(guriPacket.User.ToString(), out short slot))
                    {
                        ...
                        if (box != null)
                        {
                            if (box.Item.ItemType == ItemType.Box)
                            {
                            ...
                            else
                            {
                                ...
                                Logger.Warn("user: " + Session.Character.Name + " tried the dmg bug");
                            }
                        }
                    }
                }
                ...
                else if (guriPacket.Type == 750)
                {
                        ...
                        && Session.Character.Inventory.CountItem(baseVnum + (byte)faction) > 0)
                    {
                        if ((byte)faction < 3)
                        {
                            ...
                            if(InA4(Session.Character.MapId))
                            {
                                Session.SendPacket(UserInterfaceHelper.GenerateMsg("You cant change your Faction in A4!", 0));
                                return;
                            }
                        ...
                        else
                        {
                            ...
                            foreach(FamilyCharacter character in Session.Character.Family.FamilyCharacters)
                            {

                                if (InA4(character.Character.MapId))
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateMsg("There is still someone in A4!", 0));
                                    return;
                                }
                            }
                ...
                else if (guriPacket.Type == 4)
                {
                    ...
                    {
                        if (DateTime.Now < Session.Character.LastSpeaker.AddSeconds(30))
                            return;
                        ...
                        if (guriPacket.Value.Length > 70)
                        {
                            Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("SPEAKER_LENGTH")));
                            return;
                        }


                        Session.Character.Inventory.RemoveItemAmount(speakerVNum);
                        foreach (ClientSession client in ServerManager.Instance.Sessions.ToList())
                        {
                            if (client != null)
                                client.SendPacket(Session.Character.GenerateSay(message, 13));
                        }
                ...
                else if (guriPacket.Type == 209 && guriPacket.Argument == 0)
                {
                        ...
                        && short.TryParse(guriPacket.User.ToString(), out short pearlSlot))
                    {
                        ...
                        {
                            if (fairy.Item.EquipmentSlot != EquipmentType.Fairy)
                                return;
        ...
        public void Hero(HeroPacket heroPacket)
        {
            ...
            {
                if (Session.Character.IsReputationHero() >= 3 || Session.Character.Authority == AuthorityType.Admin)
                {
                ...
                portal)
            {
                switch (portal.Type)
                {
                    case (sbyte)PortalType.MapPortal:
                        {
                            /*if (portal.DestinationMapId == 150)
                            {
                                if(Session.Character.HeroLevel <= 39)
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("You need to be HeroLvl 40")));
                                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("You need to be HeroLvl 40"), 10));
                                    return;
                                }
                            }
                            if(portal.DestinationMapId == 153)
                            {
                                if(Session.Character.Faction == FactionType.Angel)
                                {
                                    ServerManager.Instance.ChangeMap(Session.Character.CharacterId, 153, 122, 160);
                                }
                                else
                                    ServerManager.Instance.ChangeMap(Session.Character.CharacterId, 153, 68, 156);
                            }
                            if (portal.DestinationMapId == 241)
                            {
                                if(Session.Character.HeroLevel <= 19)
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("You need to be HeroLvl 20")));
                                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("You need to be HeroLvl 20"), 10));
                                    return;
                                }


                            }
                            if (portal.DestinationMapId == 2620)
                            {
                                if (Session.Character.HeroLevel <= 59)
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("You need to be HeroLvl 60")));
                                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("You need to be HeroLvl 60"), 10));
                                    return;
                                }


                            }
                            if(portal.DestinationMapId == 232)
                            {
                                if (Session.Character.HeroLevel <= 79)
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("You need to be HeroLvl 80")));
                                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("You need to be HeroLvl 80"), 10));
                                    return;
                                }
                            }*/
                        }
            ...
            switch (rdPacket.Type)
            {
                ...
                case 1:
                    try
                    {
                        ...
                        if (rdPacket.Parameter == null && target?.Character?.Group == null && Session.Character.Group != null && Session.Character.Group.IsLeader(Session))
                        {
                            GroupJoin(new PJoinPacket
                            {
                    ...
                    {
                        Logger.Error(ex, "error comes from user: " + Session.Character.Name);
                    }
                    break;
                ...
                case 2:
                    try
                    {
                        ...
                        grp.Characters.ForEach(Session =>
                        {
        ...
        public void Say(SayPacket sayPacket)
        {
            ...
            if (!isMuted)
            {
                ...
                if (ServerManager.Instance.ChannelId == 51 && (Session.Character.Authority < AuthorityType.GameMaster || Session.Character.Undercover))
                {
                    ignoreNickname = true;
                    type = 7;
                    message = $"[!§$%&/()=?*+~#]: {message}";
                }
                else if (Session.Character.Authority == AuthorityType.Supporter)
                {
                    type = 12;
                ...
                Session.CurrentMapInstance?.Broadcast(Session, Session.Character.GenerateSay(message.Trim(), type, ignoreNickname),
                    ReceiverType.AllExceptMe);
            }
        }
        ...
        public void StartGame(GameStartPacket gameStartPacket)
        {
            ...
            if (ServerManager.Instance.ChannelId == 51 && (DateTime.Now.DayOfWeek == DayOfWeek.Monday || DateTime.Now.DayOfWeek == DayOfWeek.Thursday || DateTime.Now.DayOfWeek == DayOfWeek.Tuesday || DateTime.Now.DayOfWeek == DayOfWeek.Wednesday))
            {
            ...
            else if (Session.Character.MapInstance.Map.MapTypes.Any(m => m.MapTypeId == (short)MapTypeEnum.Act4)
            && ServerManager.Instance.ChannelId != 51 && (DateTime.Now.DayOfWeek == DayOfWeek.Monday || DateTime.Now.DayOfWeek == DayOfWeek.Tuesday || DateTime.Now.DayOfWeek == DayOfWeek.Wednesday || DateTime.Now.DayOfWeek == DayOfWeek.Thursday))
            ...
            if (ServerManager.Instance.Configuration.WorldInformation)
            {
                ...
                string productVersion = assembly?.Location != null
                    ...
                    : "1337";
                Session.SendPacket(Session.Character.GenerateSay($"----------[BlueNos]----------", 12));
                Session.SendPacket(Session.Character.GenerateSay($"Use $Warp Home       |to teleport to Nosville", 12));
                Session.SendPacket(Session.Character.GenerateSay($"Use $Buy WaterPotion |to buy cylloan water", 12));
                Session.SendPacket(Session.Character.GenerateSay($"Use $Helpme message  |to contact a crewler", 12));
                ...
                if (LastDaily == null)
                {
                    try
                    {
                        ...
                        short[] vnums = { 2801, 1024, 1025, 1026, 8109, 8112, 4350, 4351, 4552, 2514, 2515, 2516, 2517, 2518, 2519, 2520, 2521, 8283 };
                        short[] counts = { 3, 5, 5, 5, 1, 1, 1, 1, 1, 350, 350, 350, 350, 350, 350, 350, 350, 1 };
                        int item = ServerManager.RandomNumber(0, 18);
                        Session.Character.GiftAdd(vnums[item], counts[item]);

                        Session.Character.GiftAdd(5949, 1);
                        Session.SendPacket(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("GOT_DAILY"),0));
                        DAOFactory.GeneralLogDAO.Insert(GeneralLogAdd);
                    }
                    catch (Exception ex)
                    {
                        Logger.Info("unable to gift " ,ex);
                    }
                }*/
            }
            ...
            
            {
                ...
                while (true)
                {
                    string sMessage = Language.Instance.GetMessageFromKey($"BN{bCount}");
                    if (sMessage != "")
                    {
                        Session.SendPacket($"bn {bCount} {sMessage}");
                        bCount++;
                        continue;
                    }
            ...
            try
            {
                ...
                string characterName =
                    whisperPacket.Message.Split(' ')[
                            whisperPacket.Message.StartsWith("Crew ", StringComparison.CurrentCulture) ? 1 : 0]
                        .Replace("[Support]", string.Empty).Replace("[BitchNiggerFaggot]", string.Empty);
                string message = string.Empty;
                string[] packetsplit = whisperPacket.Message.Split(' ');
                for (int i = packetsplit[0] == "Crew" ? 2 : 1; i < packetsplit.Length; i++)
                {
                    ...
                    sentChannelId = CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage
                    {
                        ...
                        Message = Session.Character.Authority == AuthorityType.Supporter
                            ...
                            : Session.Character.GenerateSpk(message,
                                Session.Account.Authority >= AuthorityType.GameMaster && !Session.Character.Undercover ? 15  : 5),
