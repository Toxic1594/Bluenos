        ...
        public void CreateFamily(CreateFamilyPacket createFamilyPacket)
        {
            try
            {
                if (Session.Character.Group?.GroupType == GroupType.Group && Session.Character.Group.CharacterCount == 3)
                {
                    ...
                    {
                        Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("FAMILY_NAME_LENGTH")));
                        return;
                    }

                    Session.Character.Gold -= 200000;
                    Session.SendPacket(Session.Character.GenerateGold());
                    FactionType Faction = (FactionType)ServerManager.RandomNumber(1, 2);
                    Session.Character.Group.Characters.ForEach(s => s.Character.Faction = Faction);
                    Session.Character.Group.Characters.ForEach(s => s.SendPacket(s.Character.GenerateFaction()));
       ...
       /* public void FamilyDeposit(FDepositPacket fDepositPacket)
        {
            if (fDepositPacket != null)
            {
                ...
                lock (Session.Character.Family.__lockWareHouse)
                {
                    ...
                    ItemInstance destinationItem = Session.Character.Family.Warehouse.LoadBySlotAndType(fDepositPacket.NewSlot, InventoryType.FamilyWareHouse);
                    ...
                    ItemInstance sourceItem = Session.Character.Inventory.LoadBySlotAndType(fDepositPacket.Slot, fDepositPacket.Inventory);
                    ...
                    if (fDepositPacket.Amount < 1
                        || fDepositPacket.Amount > sourceItem.Amount)
                    {
                        return;
                    }

                    if (sourceItem.IsBound
                        || !sourceItem.Item.IsTradable)
                    {
                        return;
                    }
                    ...
                    newItem.Amount = fDepositPacket.Amount;

                    Session.Character.Inventory.RemoveItemFromInventory(sourceItem.Id, fDepositPacket.Amount);

                    Session.Character.Family.Warehouse.AddToInventoryWithSlotAndType(newItem, InventoryType.FamilyWareHouse, fDepositPacket.NewSlot);
                    Session.SendPacket(newItem.GenerateFStash());
                }
            }
        }
        */
                ...
                if (sourceInventory != null && fReposPacket.Amount <= sourceInventory.Amount)
                {
                    ...
                    else
                    {
                        if (destinationInventory.ItemVNum == sourceInventory.ItemVNum
                            && sourceInventory.Item.Type != 0)
                        {
                            if (destinationInventory.Amount + fReposPacket.Amount > 9999)
                            {
                                int saveItemCount = destinationInventory.Amount;
                                destinationInventory.Amount = 9999;
                                sourceInventory.Amount = (saveItemCount + sourceInventory.Amount - 9999);
                            }
                            else
                            {
                                destinationInventory.Amount += fReposPacket.Amount;
        ...
        public void FamilyWithdraw(FWithdrawPacket fWithdrawPacket)
        {
            if (fWithdrawPacket != null)
            {
                ...
                lock (Session.Character.Family.__lockWareHouse)
                {
                    ItemInstance sourceItem = Session.Character.Family.Warehouse.LoadBySlotAndType(fWithdrawPacket.Slot, InventoryType.FamilyWareHouse);
                    ...
                    if (fWithdrawPacket.Amount <= 0
                        || fWithdrawPacket.Amount > sourceItem.Amount)
                    {
                        return;
                    }
                    ...
                    ItemInstance newItem = sourceItem.DeepCopy();
                    newItem.Id = Guid.NewGuid();
                    newItem.Amount = fWithdrawPacket.Amount;

                    if (Session.Character.Family.RemoveItemFromFamilyWareHouse(sourceItem, fWithdrawPacket.Slot, fWithdrawPacket.Amount))
                    {
                        Session.SendPacket(Session.Character.GenerateFStashAll());
