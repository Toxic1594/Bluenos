    ...
    public class MapNpc : MapNpcDTO
    {
        ...
        private void NpcLife()
        {
            ...
            if (Target == -1)
            {
                ...
                {
                    MapMonster monster = MapInstance.Monsters.FirstOrDefault(s => s.Owner == null && MapInstance == s.MapInstance && Map.GetDistance(new MapCell { X = MapX, Y = MapY }, new MapCell { X = s.MapX, Y = s.MapY }) < (Npc.NoticeRange > 5 ? Npc.NoticeRange / 2 : Npc.NoticeRange));
