...
namespace OpenNos.GameObject
{
    public class MapInstance : BroadcastableBase
    {
        ...
        {
            Parallel.ForEach(_monsters.Where(s => s.MonsterVNum == monsterVnum),
                mapMonster =>
                {
        ...
        public MapItem PutItem(InventoryType type, short slot, int amount, ref ItemInstance inv, ClientSession session)
        {
        ...
        internal List<int> SummonMonsters(List<MonsterToSummon> monsterToSummonList)
        {
            List<int> idList = new List<int>();

            foreach (MonsterToSummon monsterToSummon
                in monsterToSummonList)
            {
                int mapMonsterId = SummonMonster(monsterToSummon);

                if (mapMonsterId != 0)
                {
                    idList.Add(mapMonsterId);
                }
            }
