...
namespace OpenNos.GameObject
{
    public class ClientSession
    {
        ...
        public bool DoInventorySave { get; set; }
            ...
            while (_receiveQueue.TryDequeue(out byte[] packetData))
            {
                ...
                foreach (string packet in packetConcatenated.Split(new[] { (char)0xFF }, StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    else
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(packetstring) || string.IsNullOrEmpty(packetstring))
                        {
                            SendPacket("info Something got Wrong you get now Disconnected!");
                            Disconnect();
                            return;
                        }
                        string packetHeader = packetstring.Split(' ')[0];

                        if(string.IsNullOrWhiteSpace(packetHeader) || string.IsNullOrEmpty(packetHeader))
                        {
                            SendPacket("info Something got Wrong you get now Disconnected!");
                            Disconnect();
                            return;
