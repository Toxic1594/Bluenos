...
namespace OpenNos.GameObject.Networking
{
    public class ServerManager : BroadcastableBase
    {
        ...
        public void ChangeMap(long id, short? mapId = null, short? mapX = null, short? mapY = null)
        {
            ...
            if (session?.Character != null)
            {
                if (mapId != null)
                {
                    if (mapId == 153)
                    {
                        session.Character.MapInstanceId = GetMapInstanceByMapId((short)mapId).MapInstanceId;
                    }
                    else
                    {
        ...
        public int GetUpgradeOrRare(ClientSession session, EquipmentType EqType, bool rare = false, bool upgrade = false)
        {
            if (session.Character.Inventory != null)
            {
                for (short i = 0; i < 15; i++)
                {
                    ItemInstance wearable = session.Character.Inventory.LoadBySlotAndType(i, InventoryType.Wear);

                    if (wearable != null)
                    {
                        if (wearable.Item.EquipmentSlot == EqType)
                        {
                            if (rare)
                            {
                                return wearable.Rare;
                            }

                            if (upgrade)
                            {
                                return wearable.Upgrade;
                            }

                            return wearable.Item.VNum;
                        }
                    }
                }
            }
        ...
        public void ChangeMapInstance(long characterId, Guid mapInstanceId, short? mapX = null, short? mapY = null, bool noAggroLoss = false)
        {
                ...
                && !session.Character.IsChangingMapInstance)
            {
                try
                {
                    ...
                    {
                        session.Character.MapInstance.Monsters.Where(m => m?.Owner?.CharacterId == session.Character.CharacterId).ToList()
                            .ForEach(mapMonster =>
                            {
                                mapMonster.SetDeathStatement();
                                mapMonster.MapInstance?.RemoveMonster(mapMonster);
                    ...
                    Parallel.ForEach(
                        ...
                        {
                            if (ChannelId != 51 || session.Character.Faction == visibleSession.Character.Faction || session.Account.Authority >= AuthorityType.GameMaster || visibleSession.Account.Authority >= AuthorityType.GameMaster)
                            {
                    ...
                    if (!session.Character.InvisibleGm)
                    {
                        ...
                        {
                            if (ChannelId != 51 || session.Character.Faction == s.Character.Faction || session.Account.Authority >= AuthorityType.GameMaster || s.Account.Authority >= AuthorityType.GameMaster)
                            {
        ...
        public void JoinMiniland(ClientSession session, ClientSession minilandOwner)
        {
            ...
            minilandOwner.Character.Mates.Where(s => !s.IsTeamMember).ToList().ForEach(s => { s.PositionX = s.MapX; s.PositionY = s.MapY; session.SendPacket(s.GenerateIn()); });
        ...
        public MapCell PetMinilandPosi() => new MapCell { X = (short)RandomNumber(5, 16), Y = (short)RandomNumber(3, 14) };
            ...
            if (session?.Character.Hp <= 0)
            {
                ...
                else
                {
                    if (ChannelId == 51)
                    {
                        session.Character.Hp = (int)session.Character.HPLoad();
                        session.Character.Mp = (int)session.Character.MPLoad();
                        if(session.CurrentMapInstance.MapInstanceType == MapInstanceType.Ac4Raid)
                        {
        ...
        private void Act4Process()
        {
            ...
            {
                MapInstanceType raidType = MapInstanceType.Ac4Raid;
        ...
        private void LaunchEvents()
        {
            ...
            //Observable.Interval(TimeSpan.FromHours(3)).Subscribe(x => BotProcess()); // Todo Optimice the BotMessages protocol
        ...
        private void MaintenanceProcess()
        {
            ...
            if (maintenanceLog != null)
            {
                if (maintenanceLog.DateStart <= DateTime.Now)
                {
                    ...
                    sessions.Where(s => s.Account.Authority < AuthorityType.Supporter).ToList().ForEach(session => session.Disconnect());
                }
            ...
            if (sender != null)
            {
                ...
                switch (message.Type)
                {
                    ...
                    case MessageType.Whisper:
                        if (targetSession == null || (message.Type == MessageType.WhisperGM && (targetSession.Account.Authority != AuthorityType.GameMaster || targetSession.Account.Authority != AuthorityType.Admin)))
                        {
                            return;
                        }
