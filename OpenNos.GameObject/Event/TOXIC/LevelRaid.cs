...
{
        public static class LevelRaid
        {
            #region Methods

            public static void GenerateLevelRaid()
            {
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTES"), 5), 0));
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTESS"), 5), 1));
                Thread.Sleep(4 * 60 * 1000);
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTES"), 1), 0));
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTES"), 1), 1));
                Thread.Sleep(30 * 1000);
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTES"), 30), 0));
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTES"), 30), 1));
                Thread.Sleep(20 * 1000);
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTES"), 10), 0));
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(string.Format(Language.Instance.GetMessageFromKey("LEVELRAID_MINUTES"), 10), 1));
                Thread.Sleep(10 * 1000);
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("LEVELRAID_STARTED"), 1));
                ServerManager.Instance.Broadcast($"qnaml 1 #guri^13371 {Language.Instance.GetMessageFromKey("LEVELRAID_QUESTION")}");
                ...
                ServerManager.Instance.StartedEvents.Remove(EventType.TOXIC);
                foreach (Tuple<MapInstance, byte> mapinstance in maps)
                {
                LevelRaidTask task = new LevelRaidTask();
                    Observable.Timer(TimeSpan.FromMinutes(0)).Subscribe(X => LevelRaidTask.Run(mapinstance));
                }
            }
                        ...
                        for (int d = 0; d < 180; d++)
                        {
                            if (!mapinstance.Item1.Monsters.Any(s => s.CurrentHp > 0))
                            {
                                ...
                                foreach (ClientSession cli in mapinstance.Item1.Sessions.Where(s => s.Character != null).ToList())
                                {
                                    ...
                                    cli.SendPacket(cli.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("WIN_FXP"), cli.Character.Level * 50), 10));
                                    cli.SendPacket(cli.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("WIN_SP_POINT"), cli.Character.Level * 450), 10));
                                }
                                break;
                            }
                            Thread.Sleep(1000);
                ...
                private static List<Tuple<short, int, short, short>> getInstantBattleDrop(Map map, short instantbattletype, int wave)
                {
                    ...
                    switch (instantbattletype)
                    {
                        ...
                        case 99:
                            ...
                            {
                                case 0:
                                    dropParameters.AddRange(generateDrop(map, 1046, 15, 250000));
                                    dropParameters.AddRange(generateDrop(map, 2282, 15, 15));
                                    dropParameters.AddRange(generateDrop(map, 1244, 25, 25));
                                    break;

                                case 1:
                                    dropParameters.AddRange(generateDrop(map, 1046, 15, 400000));
                                    dropParameters.AddRange(generateDrop(map, 2282, 20, 15));
                                    dropParameters.AddRange(generateDrop(map, 1429, 5, 5));
                                    break;

                                case 2:
                                    dropParameters.AddRange(generateDrop(map, 2282, 20, 22));
                                    dropParameters.AddRange(generateDrop(map, 1030, 20, 15));
                                    dropParameters.AddRange(generateDrop(map, 1429, 5, 5));
                                    dropParameters.AddRange(generateDrop(map, 2266, 2, 4));
                                    dropParameters.AddRange(generateDrop(map, 2815, 2, 4));
                                    break;

                                case 3:
                                    dropParameters.AddRange(generateDrop(map, 2282, 30, 30));
                                    dropParameters.AddRange(generateDrop(map, 1030, 25, 17));
                                    dropParameters.AddRange(generateDrop(map, 1429, 5, 5));
                                    dropParameters.AddRange(generateDrop(map, 2266, 3, 4));
                                    dropParameters.AddRange(generateDrop(map, 2815, 3, 4));
                                    break;
                            }
                            break;
                    }
                ...
                private static List<MonsterToSummon> getInstantBattleMonster(Map map, short instantbattletype, int wave)
                {
                    ...
                    switch (instantbattletype)
                    {
                        ...
                        case 99:
                            ...
                            {
                                case 0:
                                    SummonParameters.AddRange(map.GenerateMonsters(1042, 19, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(1048, 18, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(1047, 20, true, new List<EventContainer>()));
                                    break;

                                case 1:
                                    SummonParameters.AddRange(map.GenerateMonsters(2510, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2511, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2513, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2513, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2510, 16, true, new List<EventContainer>()));
                                    break;

                                case 2:
                                    SummonParameters.AddRange(map.GenerateMonsters(2503, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2502, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2501, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2500, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2503, 16, true, new List<EventContainer>()));
                                    break;

                                case 3:
                                    SummonParameters.AddRange(map.GenerateMonsters(2559, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2560, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2561, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2559, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2560, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2561, 16, true, new List<EventContainer>()));
                                    break;

                                case 4:
                                    SummonParameters.AddRange(map.GenerateMonsters(2574, 1, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2552, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2553, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2552, 15, true, new List<EventContainer>()));
                                    SummonParameters.AddRange(map.GenerateMonsters(2553, 15, true, new List<EventContainer>()));
                                    break;
                            }
