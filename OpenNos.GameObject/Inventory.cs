...
namespace OpenNos.GameObject
{
    public class Inventory : ThreadSafeSortedList<Guid, ItemInstance>
    {
        ...
        private const int MAX_ITEM_AMOUNT = 32000;
        ...
        public static ItemInstance InstantiateItemInstance(short vnum, long ownerId, int amount = 1)
        {
            ...
            if(newItem.Item.ItemType == ItemType.Jewelery)
                newItem.DurabilityPoint = newItem.Item.MinilandObjectPoint;

            return newItem;
        }

        public ItemInstance AddIntoBazaarInventory(InventoryType inventory, byte slot, int amount)
        {
        ...
        public List<ItemInstance> AddNewToInventory(short vnum, int amount = 1, InventoryType? type = null, sbyte Rare = 0, byte Upgrade = 0, byte Design = 0)
        {
            if (Owner != null)
            {
        ...
        public List<ItemInstance> AddToInventory(ItemInstance newItem, InventoryType? type = null)
        {
            ...
            if (Owner != null)
            {
                ...
                if (newItem.Item.Effect == 795 || newItem.Item.Effect == 794 || newItem.Item.Effect == 792 || newItem.Item.Effect == 793 || newItem.Item.Effect == 791)
                {

                    newItem.DurabilityPoint = (int)newItem.Item.EffectValue;
                }
                ...
                {
                    List<ItemInstance> slotNotFull = Where(i => i.Type != InventoryType.Bazaar && i.Type != InventoryType.PetWarehouse && i.Type != InventoryType.Warehouse && i.Type != InventoryType.FamilyWareHouse && i.ItemVNum.Equals(newItem.ItemVNum) && i.Amount < 32000);
                    int freeslot = BackpackSize() - CountLinq(s => s.Type == newItem.Type);
                    if (newItem.Amount <= (freeslot * 32000) + slotNotFull.Sum(s => 32000 - s.Amount))
                    {
                        foreach (ItemInstance slot in slotNotFull)
                        {
                            int max = slot.Amount + newItem.Amount;
                            max = max > 32000 ? 32000 : max;
                            newItem.Amount = (slot.Amount + newItem.Amount - max);
                            newItem.Amount = (newItem.Amount < 0 ? 0 : newItem.Amount);
        ...
        public void DeleteFromSlotAndType(short slot, InventoryType type)
        {
            if (Owner != null)
            {
                ...
                {
                    Logger.Log.Warn(new InvalidOperationException("Expected item wasn't deleted, Type or Slot did not match!"));
                }
            }
        }

        public ItemInstance RemoveFromWarehouse(ItemInstance itemInstance)
        {
            ...
            ItemInstance newItem = itemInstance.DeepCopy();
            newItem.Id = Guid.NewGuid();

            RemoveItemFromInventory(itemInstance.Id, itemInstance.Amount);
            AddToInventory(newItem, newItem.Item.Type);
        ...
        public bool EnoughPlace(List<ItemInstance> itemInstances)
        {
            ...
            foreach (IGrouping<short, ItemInstance> itemgroup in itemInstances.GroupBy(s => s.ItemVNum))
            {
                if (itemgroup.FirstOrDefault()?.Type is InventoryType type)
                {
                    ...
                    int rest = amount % (type == InventoryType.Equipment ? 1 : 32000);
                    bool needanotherslot = listitem.Where(s => s.ItemVNum == itemgroup.Key).Sum(s => MAX_ITEM_AMOUNT - s.Amount) <= rest;
                    place[type] -= (amount / (type == InventoryType.Equipment ? 1 : 32000)) + (needanotherslot ? 1 : 0);
        ...
        public void FDepositItem(InventoryType inventory, byte slot, int amount, byte newSlot, ref ItemInstance item, ref ItemInstance itemdest)
        {
        ...
        public void MoveItem(InventoryType sourcetype, InventoryType desttype, short sourceSlot, int amount, short destinationSlot, out ItemInstance sourceInventory, out ItemInstance destinationInventory)
        {
                ...
                else
                {
                    if (destinationInventory.ItemVNum == sourceInventory.ItemVNum && (byte)sourceInventory.Item.Type != 0)
                    {
                        if (destinationInventory.Amount + amount > MAX_ITEM_AMOUNT)
                        {
                            ...
                            sourceInventory.Amount = (saveItemCount + sourceInventory.Amount - MAX_ITEM_AMOUNT);
                        }
                        else
                        {
        ...
        public void RemoveItemFromInventory(Guid id, int amount = 1)
        {
            if (Owner != null)
            {
