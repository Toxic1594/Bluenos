...
{//gitlab test lul
    internal class CommunicationService : ScsService, ICommunicationService
    {
        ...
        public void CleanupOutdatedSession()
        {
            ...
            foreach (AccountConnection account in tmp.Where(a => a?.LastPulse.AddMinutes(5) <= DateTime.Now && ServerManager.Instance.GetSessionBySessionId(a.SessionId)?.Character != null))
            {
        ...
        public int? RegisterWorldServer(SerializableWorldServer worldServer)
        {
            ...
            if (worldServer.EndPointPort == 5003)
            {
                ws.ChannelId = 51;
            }
