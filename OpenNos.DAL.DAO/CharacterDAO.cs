...
namespace OpenNos.DAL.DAO
{
    public class CharacterDAO : ICharacterDAO
    {
        ...
        public List<CharacterDTO> GetTopReputation()
        {
            ...
            {
                List<CharacterDTO> result = new List<CharacterDTO>();
                foreach (Character entity in context.Character.Where(c => c.Account.Authority == AuthorityType.User && c.Name != "" && !c.Account.PenaltyLog.Any(l => l.Penalty == PenaltyType.Banned && l.DateEnd > DateTime.Now)).OrderByDescending(c => c.Reputation).Take(43))
                {
                    CharacterDTO dto = new CharacterDTO();
                    Mapper.Mappers.CharacterMapper.ToCharacterDTO(entity, dto);
                    result.Add(dto);
        ...
        public CharacterDTO LoadByName(string name)
        {
            try
            {
                ...
                {
                    CharacterDTO dto = new CharacterDTO();
                    if (Mapper.Mappers.CharacterMapper.ToCharacterDTO(context.Character.FirstOrDefault(c => c.Name.Equals(name)), dto))
                    {
                        return dto;
                    }
